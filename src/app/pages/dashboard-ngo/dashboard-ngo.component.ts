...
@Component({
  ...
  styleUrls: ['./dashboard-ngo.component.css'],
  providers:[DashboardNGOComponent]
})
export class DashboardNGOComponent implements OnInit {
  ...
  ngOnInit() {
    this.dashboard.getNgoDashboard().subscribe((response:any) => {
    ...
    this.userCauseDetails = this.fb.group({
  ...
  onSubmit(userCauseDetails:FormGroup){
   console.log("this.userCauseDetails.value");
      this.dashboard.postNgoDashboard(this.userCauseDetails.value).subscribe(data => {
        console.log(data);
        return data;
  })
  }
